...
{	
	...
	{				
		...
		** First blank out our result variables **/
		...
		$DeclineCode = $this->input->post('DeclineCode');
		$ExpiryDate = $this->input->post('ExpiryDate');
		$FraudResponse = $this->input->post('FraudResponse');
		$BankAuthCode = $this->input->post('BankAuthCode');
		...
		** if you need clarification on this process **/
		$Message = $VPSTxId . $VendorTxCode . $this->status . $TxAuthNo . $this->sagepay_server->config['vendorname'] . $AVSCV2 . $security_code 
		               ...
		               . $AddressStatus . $PayerStatus . $CardType . $Last4Digits . $DeclineCode . $ExpiryDate . $FraudResponse . $BankAuthCode ;
